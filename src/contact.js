...
const Contact = () => {
    ...
    const senddata = async(e) =>
    {
        ...
        const res = await fetch(
            'https://e-commerce-contact-d63bc-default-rtdb.firebaseio.com/Message.json' , options
        )
        console.log(res)
        if(res)
        {
        ...
        <div className='contant'>
            ...
            <div className='form'>
                <form method='POST'>
                    ...
                    <input type='text' name='Subject' value={users.Subject} placeholder='Enter The Subject' autoComplete='off' onChange={data}></input>
                    <textarea name='Message' value= {users.Message} placeholder='Your Message' autoComplete='off' onChange={data}></textarea>
                    {
                        isAuthenticated ?
                        <button type='submit' onClick={senddata}>SEND</button>
                        :
                        <button type='submit' onClick={()=> loginWithRedirect()}>LOG IN TO SEND</button>
